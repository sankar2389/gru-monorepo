...
class GroupDashboard extends Component<IProps, IState> {
  ...
  getCount = async (dataType: string, authtoken: string, groupID: number) => {
    const res = await axios.get(
      process.env.CMS_API + `${dataType}/count?groupID=${groupID}`,
      {
        headers: {
  ...
  updateVisiblePages = () => {
    ...
    if (
      selectedPaginatateNumber !== 1 &&
      selectedPaginatateNumber !== totalPages.length
    ) {
      const visiblePages = totalPages.slice(
        selectedPaginatateNumber - 2,
        selectedPaginatateNumber + 1
      );
      ...
      <View
        style={
          this.state.dWidth <= 700
            ? styles.smMainViewContainer
            : styles.mainViewContainer
        }
      >
        <ScrollView
          style={
            this.state.dWidth <= 700 ? styles.smInnerContainer : innerContainer
          }
        >
          ...
          <View style={styles.tableHeaderStyles}>
            <View style={{ flex: 2 }}>
              <Text style={[styles.tableColumnText, { color: "#888" }]}>
                Title
              </Text>
            </View>
